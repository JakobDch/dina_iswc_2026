...
edit distances between generated and reference queries using algebraic analysis.

Ported from chat2sparql-main with adaptations for this project.
"""
...
SIMPLE_CLAUSE_RULES = {
    ...
    "OrderBy": {
        "regex": re.compile(
            r"\bORDER\s+BY\s+(.*?)(?=\b(GROUP BY|HAVING|LIMIT|OFFSET|$))",
...
def _serialize_algebra_component(comp: Any) -> str:
    ...
    for key, value in sorted(comp_dict.items()):
        if isinstance(value, CompValue):
            ...
            serialized_elements = [
                _serialize_algebra_component(v)
...
def _extract_algebra_components(
    part: Any, components: dict[str, Any], visited: set
) -> None:
...
def _normalize_query_variables(components: dict[str, Any]) -> dict[str, Any]:
    ...
    def collect_identifiers(text: str) -> None:
    ...
    for comp_type, comp_data in components.items():
        if comp_type in ["ProjectionVars", "Triples"]:
            continue
        if isinstance(comp_data, collections.Counter):
            normalized_counter: collections.Counter[str] = collections.Counter()
...
def _get_query_components(
    query_string: str, normalize_variables: bool = True
    ...
    try:
        ...
        components: dict[str, Any] = collections.defaultdict(list)
        ...
        for key in list(components.keys()):
            if key != "ProjectionVars":
                components[key] = collections.Counter(components[key])
...
def _compare_simple_clauses_textually(
    generated_query_str: str, corrected_query_str: str, request_id: str
    ...
    for clause_name, rules in SIMPLE_CLAUSE_RULES.items():
        ...
        gen_content: Any = (
            ...
            if gen_match and rules["regex"].groups > 0
            else (gen_match is not None)
        )
        corr_content: Any = (
            corr_match.group(1).strip()
            if corr_match and rules["regex"].groups > 0
            else (corr_match is not None)
        )
...
def calculate_diff_cost(
    ...
    Args:
        generated_query_str: The generated SPARQL query
        corrected_query_str: The corrected/ground truth SPARQL query
        normalize_variables: Whether to normalize variable names
        request_id: Optional request ID for logging

    Returns:
        Dictionary with total_edits and edits breakdown, or None on error
    """
    log_prefix = f"[{request_id}]" if request_id else ""
    logger.debug(f"{log_prefix} Starting diff_cost calculation.")

    cand_comps = _get_query_components(generated_query_str, normalize_variables)
    corr_comps = _get_query_components(corrected_query_str, normalize_variables)

    if not cand_comps or not corr_comps:
        logger.error(
            f"{log_prefix} Could not extract components from one or both queries."
        )
        return None

    total_cost = 0
    edits_breakdown: list[dict[str, Any]] = []
    ...
    added_triples_details = [
        " ".join(t_corr)
        for i, t_corr in enumerate(corr_triples)
        if i not in matched_corr_indices
    ]
    ...
    algebra_types_to_check = {
        k: v
        for k, v in ALGEBRA_COST_RULES.items()
        if k not in ["Distinct", "OrderBy", "Slice"]
    }
...
def calculate_max_edit_cost(
    query_string: str, request_id: str = ""
) -> Optional[dict[str, Any]]:
    """
    ...
    Args:
        query_string: The SPARQL query to analyze
        request_id: Optional request ID for logging

    Returns:
        Dictionary with max_edit_cost and cost_breakdown, or None on error
    """
    log_prefix = f"[{request_id}]" if request_id else ""
    logger.debug(f"{log_prefix} Starting max_edit_cost calculation.")
    ...
    if num_triples > 0:
        ...
        cost_breakdown.append(
            {"component": "Triples", "cost": cost, "detail": f"{num_triples} triples"}
        )
    ...
    if num_vars > 0:
        ...
        cost_breakdown.append(
            {
                "component": "Projection Variables",
                "cost": cost,
                "detail": f"{num_vars} variables",
            }
        )

    for comp_type, rule_cost in ALGEBRA_COST_RULES.items():
        if rule_cost > 0:
            comp_data = corr_comps.get(comp_type)
            if isinstance(comp_data, collections.Counter):
                count = sum(comp_data.values())
            ...
            if count > 0:
                ...
                cost_breakdown.append(
                    {
                        "component": f"Structure: {comp_type}",
                        "cost": cost,
                        "detail": f"{count} instance(s)",
                    }
                )
...
def calculate_edit_cost_score(
    ...
    """
    Calculate the normalized edit cost score between two SPARQL queries.

    This is the main function to use for evaluation. It returns a normalized
    score between 0 and 100, where 100 means the queries are identical.
    ...
    """
    diff_result = calculate_diff_cost(
        generated_query, corrected_query, normalize_variables, request_id
    )
    max_result = calculate_max_edit_cost(corrected_query, request_id)

    if diff_result is None or max_result is None:
        return {
            "actual_cost": 0,
            "max_cost": 0,
            "normalized_score": 0.0,
            "edit_details": [],
            "error": "Failed to parse one or both queries",
        }

    actual_cost = diff_result.get("total_edits", 0)
    max_cost = max_result.get("max_edit_cost", 1)

    if max_cost > 0:
        normalized_score = max(0.0, (1.0 - (float(actual_cost) / float(max_cost))) * 100.0)
    else:
        normalized_score = 100.0 if actual_cost == 0 else 0.0

    return {
        "actual_cost": actual_cost,
        "max_cost": max_cost,
        "normalized_score": round(normalized_score, 2),
        "edit_details": diff_result.get("edits", []),
    }
