...
Uses embedding-based search to find relevant schema information.
This agent ONLY has access to semantic/embedding-based tools for fair comparison.
...
def emit_agent_message(tracer, msg: BaseMessage, agent_name: str = "semantic_retrieval_agent") -> None:
...
SYSTEM_PROMPT = """You are a schema retrieval agent that extracts relevant tripels from semantic models using semantic search.
...
1. **search_classes(query, top_k?)** - Find classes by semantic similarity. Returns hierarchy info.
2. **search_properties(query, top_k?)** - Find properties by semantic similarity (e.g., "rating", "price", "connects to").
...
5. **search_data_values(class, property, term, dataset?, prefix?)** - Find actual data values (semantic search).
...
- **CONCEPTS** (students, books, trips) → search_classes()
- **NAMED ENTITIES** (person names, company names, place names) → search_data_values()
- **NUMERIC FILTERS** (under 50, in 2023) → schema only, SPARQL handles these

Named entities are NOT in schema - use search_data_values() to find actual values!

### Step 2: Find Classes and Connections
```
search_classes("Book")  → Find Book class
search_classes("Author") → Find Author class
find_shortest_path(["Book", "Author"]) → Get connection tripels
```

**IMPORTANT**: Call `find_shortest_path` for ALL pairs of found classes within the same dataset. Every class must be connected to every other class — unconnected classes are useless for SPARQL generation. Classes from different datasets do not need to be connected.
...
```
search_data_values("lib:Author", "lib:name", "Shakespeare", dataset="LIBRARY")
...
- **Hierarchy**: search_classes() returns parent/child classes. Use get_class_hierarchy() for details.
...
class SemanticRetrievalAgent:
    """Agent that uses embedding-based search for schema retrieval.
    ...
    def __init__(self, llm_model: str = "deepseek-chat"):
        ...
        self.tools = SEMANTIC_TOOLS
        self.llm_with_tools = self.llm.bind_tools(self.tools)
        self.tracer = get_tracer()
        self.llm_callback = create_llm_callback("retrieval", "semantic_search")

        # Ensure schema is indexed for all datasets
    ...
    def _ensure_indexed(self) -> int:
        """Ensure schema is indexed for all datasets (Dataspace mode)."""
        index = get_schema_index()
        if not index.is_global_indexed():
            return index.index_all_datasets()
        return len(index._elements)
    ...
    async def retrieve(
        ...
        """
        Retrieve relevant schema information using semantic search.
...
"""
            ...
            emit_agent_message(self.tracer, sys_msg, "semantic_retrieval_agent")
            emit_agent_message(self.tracer, human_msg, "semantic_retrieval_agent")
        ...
        while validation_attempt < max_validation_attempts:
            ...
            while tool_call_count < max_tool_calls:
                ...
                emit_agent_message(self.tracer, response, "semantic_retrieval_agent")
                ...
                for tool_call in response.tool_calls:
                    ...
                    if tool_call_count >= max_tool_calls:
                        ...
                        emit_agent_message(self.tracer, limit_msg, "semantic_retrieval_agent")
                        continue
                    ...
                    if not tool_fn:
                        ...
                        emit_agent_message(self.tracer, not_found_msg, "semantic_retrieval_agent")
                        continue
                    ...
                    try:
                        ...
                        emit_agent_message(self.tracer, result_tool_msg, "semantic_retrieval_agent")
                        ...
                        emit_agent_message(self.tracer, error_tool_msg, "semantic_retrieval_agent")
...
You CANNOT make more tool calls. Decide NOW based on what you already found."""
                ...
                emit_agent_message(self.tracer, force_human_msg, "semantic_retrieval_agent")
                ...
                emit_agent_message(self.tracer, forced_response, "semantic_retrieval_agent")
            ...
            if validation_result.is_valid:
                ...
                if validation_attempt < max_validation_attempts:
                    ...
                    emit_agent_message(self.tracer, validation_feedback_msg, "semantic_retrieval_agent")
                else:
    ...
    def _extract_metadata(
        ...
        if tool_name == "search_classes" and isinstance(result, list):
            for item in result:
                if isinstance(item, dict):
                    for f in item.get("defined_in", []):
                        mapping_files.add(f)
                    if item.get("sparql_endpoint"):
                        detected_endpoints.add(item["sparql_endpoint"])

        elif tool_name == "search_properties" and isinstance(result, list):
            for item in result:
                if isinstance(item, dict):
                    for f in item.get("defined_in", []):
                        mapping_files.add(f)
        ...
        elif tool_name == "search_data_values" and isinstance(result, dict):
