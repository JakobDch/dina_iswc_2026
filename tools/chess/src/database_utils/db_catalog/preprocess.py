...
if VertexAIEmbeddings and GCP_CREDENTIALS and GCP_PROJECT and GCP_REGION:
    aiplatform.init(
    project=GCP_PROJECT,
    location=GCP_REGION,
...
def make_db_context_vec_db(db_directory_path: str, **kwargs) -> None:
    ...
    # ChromaDB has a max batch size of 5461; insert in chunks if needed
    BATCH_SIZE = 5000
    if len(docs) <= BATCH_SIZE:
        Chroma.from_documents(docs, EMBEDDING_FUNCTION, persist_directory=str(vector_db_path))
    else:
        # Create DB with first batch, then add remaining batches
        db = Chroma.from_documents(docs[:BATCH_SIZE], EMBEDDING_FUNCTION, persist_directory=str(vector_db_path))
        for i in range(BATCH_SIZE, len(docs), BATCH_SIZE):
            batch = docs[i:i + BATCH_SIZE]
            logging.info(f"Adding batch {i // BATCH_SIZE + 1} ({len(batch)} docs)")
            db.add_documents(batch)

    logging.info(f"Context vector database created at {vector_db_path} ({len(docs)} docs)")
